...
import media_quality_checker as mqc
...
def test_acquire_lock_file_reclaims_stale_dead_owner(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mqc, "_pid_is_running", lambda _pid: False)

    fd, token = mqc._acquire_lock_file(str(lock_path), timeout_s=0.5)
    try:
        assert token.startswith(f"{os.getpid()}:")
    finally:
        mqc._release_lock_file(str(lock_path), fd, token)
...
def test_acquire_lock_file_does_not_steal_live_owner_even_if_old(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mqc, "_pid_is_running", lambda _pid: True)

    with pytest.raises(TimeoutError):
        mqc._acquire_lock_file(str(lock_path), timeout_s=0.05)
