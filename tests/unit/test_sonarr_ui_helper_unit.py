...
from arr_helper.sonarr_ui import api as sui_api
from arr_helper.sonarr_ui import probe_cache as sui_probe
from arr_helper.sonarr_ui import workers as sui_workers
...
def test_sonarr_api_uses_configured_timeout(monkeypatch):
    ...
    monkeypatch.setattr(sui_api.requests, "get", _mk("get"))
    monkeypatch.setattr(sui_api.requests, "post", _mk("post"))
    monkeypatch.setattr(sui_api.requests, "put", _mk("put"))
    monkeypatch.setattr(sui_api.requests, "delete", _mk("delete"))

    api = sui_api.SonarrAPI("http://x", "k", request_timeout=(1, 2))
    api.get_series()
...
def probe_cache_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sui_probe, "_PROBE_CACHE_PATH", str(cache_path))
    monkeypatch.setattr(sui_probe, "_PROBE_CACHE_LOCK_PATH", str(lock_path))
    monkeypatch.setattr(sui_probe, "_probe_cache", {})
...
def test_probe_file_uses_cache_without_subprocess(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sui_probe, "_probe_cache", {str(media): cached})

    def _no_run(*args, **kwargs):
        raise AssertionError("subprocess.run should not be called for cache hits")

    monkeypatch.setattr(sui_probe.subprocess, "run", _no_run)

    assert sui_probe.probe_file(str(media)) == cached
...
def test_probe_file_parses_ffprobe_json(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sui_probe.subprocess, "run", lambda *a, **k: _RunResult())

    result = sui_probe.probe_file(str(media))
...
def test_probe_file_does_not_cache_failed_probe(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sui_probe.subprocess, "run", _run)

    first = sui_probe.probe_file(str(media))
    second = sui_probe.probe_file(str(media))
...
def test_loadworker_emits_error_and_empty_when_series_fetch_fails():
    worker = sui_workers.LoadWorker(_APIGetSeriesError())
    progress = []
    ready = []
...
def test_loadworker_reports_warnings_and_finishes(monkeypatch):
    worker = sui_workers.LoadWorker(_APIWithWarnings())
    progress = []
    ready = []
    worker.progress.connect(progress.append)
    worker.series_ready.connect(ready.append)
...
def test_loadworker_unexpected_exception_is_reported():
    worker = sui_workers.LoadWorker(_APIBadShape())
    progress = []
    ready = []
    worker.progress.connect(progress.append)
    worker.series_ready.connect(ready.append)
